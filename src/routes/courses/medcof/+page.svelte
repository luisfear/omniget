<script lang="ts">
  ...
  let courses: MedcofCourse[] = $state([]);
  ...
  async function checkSession() {
    try {
      const result = await invoke<string>("medcof_check_session");
      sessionEmail = result;
      loggedIn = true;
      loadCourses();
    } catch {
  ...
  async function handleTokenLogin() {
    ...
    loading = true;
    try {
      const result = await invoke<string>("medcof_login_token", { token: token.trim() });
  ...
  async function loadCourses() {
    ...
    coursesError = "";
    try {
      courses = await invoke("medcof_list_courses");
      currentPage = 1;
    } catch (e: any) {
  ...
  async function downloadCourse(course: MedcofCourse) {
    ...
    try {
      await invoke("start_medcof_download", {
  ...
  async function refreshCourses() {
    ...
    coursesError = "";
    try {
      courses = await invoke("medcof_refresh_courses");
      currentPage = 1;
    } catch (e: any) {
      coursesError = typeof e === "string" ? e : e.message ?? $t('hotmart.courses_error');
    } finally {
...
{:else}
  <div class="page-center">
    <div class="login-card">
      ...
      <div class="form">
        <label class="field">
          <span class="field-label">JWT Token (aulas_auth_token cookie)</span>
          <textarea
            class="input token-textarea"
            placeholder="Paste your aulas_auth_token cookie here..."
            bind:value={token}
            disabled={loading}
            rows="4"
          ></textarea>
