<script lang="ts">
  ...
  async function checkSession() {
    try {
      const result = await invoke<string>("areademembros_check_session");
      sessionEmail = result;
      loggedIn = true;
      loadCourses();
    } catch {
  ...
  async function handleTokenLogin() {
    ...
    loading = true;
    try {
      const result = await invoke<string>("areademembros_login_token", { token: token.trim(), siteUrl: siteUrl.trim() });
  ...
  async function handleLogout() {
    try {
      await invoke("areademembros_logout");
    } catch {
    }
    loggedIn = false;
    sessionEmail = "";
  ...
  async function loadCourses() {
    ...
    coursesError = "";
    try {
      courses = await invoke("areademembros_list_courses");
      currentPage = 1;
    } catch (e: any) {
  ...
  async function downloadCourse(course: AreaDeMembrosApiCourse) {
    ...
    try {
      await invoke("start_areademembros_download", {
  ...
  async function refreshCourses() {
    ...
    coursesError = "";
    try {
      courses = await invoke("areademembros_refresh_courses");
      currentPage = 1;
    } catch (e: any) {
      coursesError = typeof e === "string" ? e : e.message ?? $t('hotmart.courses_error');
    } finally {
  ...
  <div class="page-center">
    <div class="login-card">
      ...
      <div class="form">
        ...
        <label class="field">
          <span class="field-label">Session Cookies</span>
          <textarea
            class="input token-textarea"
            placeholder="Paste your session cookies here..."
            bind:value={token}
            disabled={loading}
            rows="4"
          ></textarea>
