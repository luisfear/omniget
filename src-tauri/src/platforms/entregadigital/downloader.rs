...
use super::api::{self, EntregaDigitalCourse, EntregaDigitalSession};

#[derive(Debug, Clone, serde::Serialize)]
pub struct EntregaDigitalCourseDownloadProgress {
    pub course_id: i64,
...
pub async fn download_full_course(
    ...
    let _ = app.emit(
        "download-progress",
        &EntregaDigitalCourseDownloadProgress {
            course_id: course.id,
    ...
    for (mi, module) in modules.iter().enumerate() {
        ...
        for (li, lesson) in module.lessons.iter().enumerate() {
            ...
            let detail = match api::get_lesson_detail(session, lesson.id).await {
                ...
                Err(e) => {
                    tracing::error!(
                        "[entregadigital] Failed to get lesson detail '{}': {}. Continuing...",
                        lesson.name,
                        e
                    );
            ...
            if let Some(ref video_url) = detail.video_url {
                ...
                if tokio::fs::try_exists(&video_path).await.unwrap_or(false) {
                    ...
                    if meta.map(|m| m.len() > 0).unwrap_or(false) {
                        tracing::info!("[entregadigital] Skipping existing: {}", video_path);
                        let done = completed.fetch_add(1, Ordering::Relaxed) + 1;
                        let _ = app.emit(
                            "download-progress",
                            &EntregaDigitalCourseDownloadProgress {
                ...
                if video_url.contains(".m3u8") {
                    match download_hls_video(video_url, &video_path, &cancel_token).await {
                        ...
                        Err(e) => {
                            tracing::error!(
                                "[entregadigital] HLS download failed for '{}': {}",
                                lesson.name,
                                e
                            );
                    ...
                    match download_with_ytdlp(video_url, &lesson_dir, &cancel_token).await {
                        ...
                        Err(e) => {
                            tracing::error!(
                                "[entregadigital] yt-dlp download failed for '{}': {}, trying direct download",
                                lesson.name,
                                e
                            );
                            match download_file_direct(&session.client, video_url, &video_path, &cancel_token).await {
                                ...
                                Err(e2) => {
                                    tracing::error!(
                                        "[entregadigital] Direct download also failed for '{}': {}",
                                        lesson.name,
                                        e2
                                    );
                                }
            ...
            for attachment in &detail.attachments {
                ...
                match download_file_direct(&session.client, &attachment.url, &att_path, &cancel_token).await {
                    ...
                    Err(e) => {
                        tracing::error!(
                            "[entregadigital] Attachment download failed for '{}': {}",
                            attachment.name,
                            e
                        );
                    }
...
async fn download_with_ytdlp(
    ...
    let result = crate::core::ytdlp::download_video(
        ...
        None,
        None,
        Some("https://entregadigital.app.br/"),
        cancel_token.clone(),
        None,
        8,
        false,
