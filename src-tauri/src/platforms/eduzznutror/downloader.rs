...
use super::api::{self, NutrorCourse, NutrorSession};

#[derive(Debug, Clone, serde::Serialize)]
pub struct NutrorCourseDownloadProgress {
...
pub async fn download_full_course(
    ...
    let modules = api::get_modules(session, &course.hash).await?;
    ...
    if crate::core::course_utils::is_course_complete(&course_dir) {
        tracing::info!("[nutror] course already complete, skipping");
        return Ok(());
    }
    ...
    for (mi, module) in modules.iter().enumerate() {
        ...
        for (li, lesson) in module.lessons.iter().enumerate() {
            ...
            let detail = match api::get_lesson_detail(session, &lesson.hash).await {
                Ok(d) => d,
                Err(e) => {
                    tracing::error!(
                        "[nutror] Failed to get lesson detail '{}': {}. Continuing...",
                        lesson.name,
                        e
                    );
            ...
            if let Some(ref video_url) = detail.video_url {
                ...
                if tokio::fs::try_exists(&video_path).await.unwrap_or(false) {
                    ...
                    if meta.map(|m| m.len() > 0).unwrap_or(false) {
                        tracing::info!("[nutror] Skipping existing: {}", video_path);
                        let done = completed.fetch_add(1, Ordering::Relaxed) + 1;
                        let _ = app.emit(
                            "download-progress",
                            &NutrorCourseDownloadProgress {
                ...
                if video_url.contains(".m3u8") {
                    match download_hls_video(video_url, &video_path, &cancel_token).await {
                        ...
                        Err(e) => {
                            tracing::error!(
                                "[nutror] HLS download failed for '{}': {}",
                                lesson.name,
                                e
                            );
                    ...
                    match download_with_ytdlp(video_url, &lesson_dir, &cancel_token).await {
                        ...
                        Err(e) => {
                            tracing::error!(
                                "[nutror] yt-dlp download failed for '{}': {}, trying direct download",
                                lesson.name,
                                e
                            );
                            match download_file_direct(&session.client, video_url, &video_path, &cancel_token).await {
                                ...
                                Err(e2) => {
                                    tracing::error!(
                                        "[nutror] Direct download also failed for '{}': {}",
                                        lesson.name,
                                        e2
                                    );
                                }
            ...
            for file in &detail.files {
                ...
                match download_file_direct(&session.client, &file.url, &file_path, &cancel_token).await {
                    ...
                    Err(e) => {
                        tracing::error!(
                            "[nutror] File download failed for '{}': {}",
                            file.name,
                            e
                        );
                    }
