...
use super::api::{self, KiwifyCourse, KiwifySession};
    ...
    for (mi, module) in modules.iter().enumerate() {
        ...
        for (li, lesson) in module.lessons.iter().enumerate() {
            ...
            let detail = match api::get_lesson_detail(session, course.id, &lesson.id).await {
                ...
                Err(e) => {
                    tracing::error!(
                        "[kiwify] Failed to get lesson detail '{}': {}. Continuing...",
                        lesson.name,
                        e
                    );
            ...
            if let Some(ref video_url) = detail.video_url {
                ...
                if video_url.contains(".m3u8") {
                    match download_hls_video(video_url, &video_path, &cancel_token).await {
                        ...
                        Err(e) => {
                            tracing::error!(
                                "[kiwify] HLS download failed for '{}': {}",
                                lesson.name,
                                e
                            );
                    ...
                    match download_with_ytdlp(video_url, &lesson_dir, &cancel_token).await {
                        ...
                        Err(e) => {
                            tracing::error!(
                                "[kiwify] yt-dlp download failed for '{}': {}, trying direct download",
                                lesson.name,
                                e
                            );
                            match download_file_direct(&session.client, video_url, &video_path, &cancel_token).await {
                                ...
                                Err(e2) => {
                                    tracing::error!(
                                        "[kiwify] Direct download also failed for '{}': {}",
                                        lesson.name,
                                        e2
                                    );
                                }
            ...
            for file in &detail.files {
                ...
                let download_url = if let Some(ref url) = file.url {
                    ...
                    match api::get_file_download_url(session, course.id, &file.id).await {
                        ...
                        Err(e) => {
                            tracing::error!(
                                "[kiwify] Failed to get download URL for file '{}': {}",
                                file.name,
                                e
                            );
                            continue;
...
async fn download_with_ytdlp(
    ...
    let result = crate::core::ytdlp::download_video(
        ...
        None,
        None,
        Some("https://admin.kiwify.com.br/"),
        cancel_token.clone(),
        None,
        8,
        false,
