...
use super::api::{self, MedcelCourse, MedcelSession};

#[derive(Debug, Clone, serde::Serialize)]
pub struct MedcelCourseDownloadProgress {
...
pub async fn download_full_course(
    ...
    let _ = app.emit(
        "medcel-download-progress",
        &MedcelCourseDownloadProgress {
        ...
        for (li, lesson) in module.lessons.iter().enumerate() {
            ...
            if lesson.type_id == "videoClass" || lesson.type_id.contains("video") {
                match api::get_video_url(session, &lesson.id, &lesson.product_id).await {
                    Ok(video_url) => {
                        ...
                        if tokio::fs::try_exists(&video_path).await.unwrap_or(false) {
                            ...
                            if meta.map(|m| m.len() > 0).unwrap_or(false) {
                                tracing::info!("[medcel] Skipping existing: {}", video_path);
                        ...
                        if video_url.contains(".m3u8") {
                            match download_hls_video(&video_url, &video_path, &cancel_token).await {
                                ...
                                Err(e) => {
                                    tracing::error!(
                                        "[medcel] HLS download failed for '{}': {}",
                                        lesson.name,
                                        e
                                    );
                            ...
                            match download_with_ytdlp(&video_url, &lesson_dir, &cancel_token).await {
                                ...
                                Err(e) => {
                                    tracing::error!(
                                        "[medcel] yt-dlp download failed for '{}': {}, trying direct download",
                                        lesson.name,
                                        e
                                    );
                                    match download_file_direct(&session.client, &video_url, &video_path, &cancel_token).await {
                                        ...
                                        Err(e2) => {
                                            tracing::error!(
                                                "[medcel] Direct download also failed for '{}': {}",
                                                lesson.name,
                                                e2
                                            );
                                        }
                    ...
                    Err(e) => {
                        tracing::error!(
                            "[medcel] Failed to get video URL for '{}': {}",
                            lesson.name,
                            e
                        );
                    }
            ...
            for material in &lesson.materials {
                ...
                match download_file_direct(&session.client, &material.url, &mat_path, &cancel_token).await {
                    ...
                    Err(e) => {
                        tracing::error!(
                            "[medcel] Material download failed for '{}': {}",
                            material.title,
                            e
                        );
                    }
...
async fn download_with_ytdlp(
    ...
    let result = crate::core::ytdlp::download_video(
        ...
        None,
        None,
        Some("https://areaaluno.medcel.com.br/"),
        cancel_token.clone(),
        None,
        8,
        false,
