...
use super::api::{self, TheMembersCourse, TheMembersSession};

#[derive(Debug, Clone, serde::Serialize)]
pub struct TheMembersCourseDownloadProgress {
    pub course_id: i64,
...
pub async fn download_full_course(
    ...
    let _ = app.emit(
        "download-progress",
        &TheMembersCourseDownloadProgress {
            course_id: course.id,
    ...
    for (mi, module) in modules.iter().enumerate() {
        ...
        for (li, lesson) in module.lessons.iter().enumerate() {
            ...
            let detail = match api::get_lesson_detail(session, lesson.id).await {
                ...
                Err(e) => {
                    tracing::error!(
                        "[themembers] Failed to get lesson detail '{}': {}. Continuing...",
                        lesson.name,
                        e
                    );
            ...
            if let Some(ref pdf_url) = detail.url_pdf {
                crate::core::course_utils::download_attachment(
                    &session.client,
                    pdf_url,
                    &lesson_dir,
                    &format!("{}.pdf", lesson_name),
                    &cancel_token,
                ).await.ok();
            }
            ...
            if let Some(ref video_url) = detail.video_url {
                ...
                if tokio::fs::try_exists(&video_path).await.unwrap_or(false) {
                    ...
                    if meta.map(|m| m.len() > 0).unwrap_or(false) {
                        tracing::info!("[themembers] Skipping existing: {}", video_path);
                        let done = completed.fetch_add(1, Ordering::Relaxed) + 1;
                        let _ = app.emit(
                            "download-progress",
                            &TheMembersCourseDownloadProgress {
                ...
                let referer = format!("https://{}/", session.domain);

                if video_url.contains(".m3u8") {
                    match download_hls_video(video_url, &video_path, &referer, &cancel_token).await {
                        Ok(size) => {
                            total_bytes.fetch_add(size, Ordering::Relaxed);
                        }
                        Err(e) => {
                            tracing::error!(
                                "[themembers] HLS download failed for '{}': {}",
                                lesson.name,
                                e
                            );
                    ...
                    match download_with_ytdlp(video_url, &lesson_dir, &referer, &cancel_token).await {
                        Ok(size) => {
                            total_bytes.fetch_add(size, Ordering::Relaxed);
                        }
                        Err(e) => {
                            tracing::error!(
                                "[themembers] yt-dlp download failed for '{}': {}, trying direct download",
                                lesson.name,
                                e
                            );
                            match download_file_direct(&session.client, video_url, &video_path, &cancel_token).await {
                                ...
                                Err(e2) => {
                                    tracing::error!(
                                        "[themembers] Direct download also failed for '{}': {}",
                                        lesson.name,
                                        e2
                                    );
                                }
            ...
            for file in &detail.files {
                ...
                match download_file_direct(&session.client, &file.url, &file_path, &cancel_token).await {
                    ...
                    Err(e) => {
                        tracing::error!(
                            "[themembers] File download failed for '{}': {}",
                            file.name,
                            e
                        );
                    }
