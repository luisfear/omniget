...
use super::api::{self, KajabiCourse, KajabiSession};

#[derive(Debug, Clone, serde::Serialize)]
pub struct KajabiCourseDownloadProgress {
...
pub async fn download_full_course(
    ...
    let modules = api::get_categories(session, &course.id).await?;
    ...
    for (mi, module) in modules.iter().enumerate() {
        ...
        for (li, lesson) in module.lessons.iter().enumerate() {
            ...
            let detail = match api::get_post_detail(session, &course.id, &lesson.id).await {
                Ok(d) => d,
                Err(e) => {
                    tracing::error!(
                        "[kajabi] Failed to get post detail '{}': {}. Continuing...",
                        lesson.name,
                        e
                    );
            ...
            if let Some(ref video_url) = detail.video_url {
                ...
                if video_url.contains(".m3u8") {
                    match download_hls_video(video_url, &video_path, &cancel_token).await {
                        ...
                        Err(e) => {
                            tracing::error!(
                                "[kajabi] HLS download failed for '{}': {}",
                                lesson.name,
                                e
                            );
                    ...
                    match download_with_ytdlp(video_url, &lesson_dir, &cancel_token).await {
                        ...
                        Err(e) => {
                            tracing::error!(
                                "[kajabi] yt-dlp download failed for '{}': {}, trying direct download",
                                lesson.name,
                                e
                            );
                            match download_file_direct(&session.client, video_url, &video_path, &cancel_token).await {
                                ...
                                Err(e2) => {
                                    tracing::error!(
                                        "[kajabi] Direct download also failed for '{}': {}",
                                        lesson.name,
                                        e2
                                    );
                                }
            ...
            for file in &detail.files {
                ...
                if let Some(ref url) = file.url {
                    match download_file_direct(&session.client, url, &file_path, &cancel_token).await {
                        Ok(size) => {
                            total_bytes.fetch_add(size, Ordering::Relaxed);
                        }
                        Err(e) => {
                            tracing::error!(
                                "[kajabi] File download failed for '{}': {}",
                                file.name,
                                e
                            );
                        }
