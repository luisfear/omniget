...
use super::api::{self, KirvanoCourse, KirvanoSession};

#[derive(Debug, Clone, serde::Serialize)]
pub struct KirvanoCourseDownloadProgress {
...
pub async fn download_full_course(
    ...
    course: &KirvanoCourse,
    output_dir: &str,
    cancel_token: CancellationToken,
) -> anyhow::Result<()> {
    let modules = api::get_course_modules(session, &course.course_uuid).await?;
        ...
        for (li, lesson) in module.lessons.iter().enumerate() {
            ...
            if let Some(ref video_url) = lesson.video_url {
                ...
                match download_with_ytdlp(video_url, &mod_dir, &cancel_token).await {
                    ...
                    Err(e) => {
                        tracing::error!(
                            "[kirvano] yt-dlp download failed for '{}': {}, trying direct download",
                            lesson.name,
                            e
                        );
                        match download_file_direct(&session.client, video_url, &video_path, &cancel_token).await {
                            ...
                            Err(e2) => {
                                tracing::error!(
                                    "[kirvano] Direct download also failed for '{}': {}",
                                    lesson.name,
                                    e2
                                );
                            }
