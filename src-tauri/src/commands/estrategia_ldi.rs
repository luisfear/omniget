...
use crate::platforms::estrategia_ldi::api::{self, EstrategiaLdiCourse, EstrategiaLdiSession};
use crate::platforms::estrategia_ldi::downloader;
use crate::AppState;
...
struct EstrategiaLdiDownloadCompleteEvent {
    course_name: String,
    success: bool,
    error: Option<String>,
}
...
#[tauri::command]
pub async fn estrategia_ldi_login_token(
    state: tauri::State<'_, AppState>,
    token: String,
) -> Result<String, String> {
    let _ = api::delete_saved_session().await;
    state.estrategia_ldi_session.lock().await.take();
    *state.estrategia_ldi_session_validated_at.lock().await = None;
    *state.estrategia_ldi_courses_cache.lock().await = None;
    ...
    let session = EstrategiaLdiSession {
    ...
    match api::validate_token(&session).await {
        Ok(true) => {
            ...
            let mut guard = state.estrategia_ldi_session.lock().await;
            *guard = Some(session);
            *state.estrategia_ldi_session_validated_at.lock().await = Some(Instant::now());
...
#[tauri::command]
pub async fn estrategia_ldi_check_session(
    state: tauri::State<'_, AppState>,
) -> Result<String, String> {
    let has_memory_session = state.estrategia_ldi_session.lock().await.is_some();
    ...
    let guard = state.estrategia_ldi_session.lock().await;
    let session = guard
        .as_ref()
        .ok_or_else(|| "not_authenticated".to_string())?;

    {
        let validated_at = state.estrategia_ldi_session_validated_at.lock().await;
...
#[tauri::command]
pub async fn estrategia_ldi_logout(
    state: tauri::State<'_, AppState>,
) -> Result<(), String> {
    let _ = api::delete_saved_session().await;
    state.estrategia_ldi_session.lock().await.take();
...
async fn fetch_estrategia_ldi_courses(
    state: &tauri::State<'_, AppState>,
) -> Result<Vec<EstrategiaLdiCourse>, String> {
    let guard = state.estrategia_ldi_session.lock().await;
    let session = guard
        .as_ref()
    ...
    let mut cache = state.estrategia_ldi_courses_cache.lock().await;
    *cache = Some(crate::EstrategiaLdiCoursesCache {
...
#[tauri::command]
pub async fn estrategia_ldi_list_courses(
    state: tauri::State<'_, AppState>,
) -> Result<Vec<EstrategiaLdiCourse>, String> {
    {
        let cache = state.estrategia_ldi_courses_cache.lock().await;
    ...
    fetch_estrategia_ldi_courses(&state).await
}

#[tauri::command]
pub async fn estrategia_ldi_refresh_courses(
    state: tauri::State<'_, AppState>,
) -> Result<Vec<EstrategiaLdiCourse>, String> {
    {
        let mut cache = state.estrategia_ldi_courses_cache.lock().await;
...
#[tauri::command]
pub async fn start_estrategia_ldi_course_download(
    app: tauri::AppHandle,
    state: tauri::State<'_, AppState>,
    course_json: String,
    output_dir: String,
) -> Result<String, String> {
    let course: EstrategiaLdiCourse =
    ...
    tokio::spawn(async move {
        ...
        match result {
            Ok(()) => {
                let _ = app.emit(
                    "download-complete",
                    &EstrategiaLdiDownloadCompleteEvent {
            ...
            Err(e) => {
                tracing::error!("[estrategia_ldi] download error for '{}': {}", course.name, e);
                let _ = app.emit(
                    "download-complete",
                    &EstrategiaLdiDownloadCompleteEvent {
                        course_name: course.name,
